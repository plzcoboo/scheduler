...
<%
    ...
    if ("POST".equalsIgnoreCase(request.getMethod()) &&
        ...
        try {
            ...
            // SQL 쿼리 작성
            String finPwSql = "SELECT pw FROM users WHERE role_idx = ? AND department_idx = ? AND name = ? AND account = ? AND phone_number = ?";
            inputQuery = conn.prepareStatement(finPwSql);
            inputQuery.setInt(1, roleIdx);
            inputQuery.setInt(2, departmentIdx);
            inputQuery.setString(3, name);
            inputQuery.setString(4, userId);
            inputQuery.setString(5, phoneNumber);
            // 쿼리 실행
            rs = inputQuery.executeQuery();
            if (rs.next()) {
                String userPw = rs.getString("pw");
                message = "귀하의 비밀번호는: " + userPw + "입니다.";
            } else {
...
<head>
    ...
    <link rel="stylesheet" href="../css/find_user_pw.css">
</head>
<body>
    <main>
        <h1 class="logo">
